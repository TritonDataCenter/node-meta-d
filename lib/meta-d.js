/*
 * meta-d: Provides an implementation for transforming a JSON representation
...
var mod_dutil = require('./dutil.js');
...
/*
 * Defines the maximum number of zones for which we'll use per-zone scripts
 * rather than a single instrumentation predicated on the zonename.
 */
...
/*
 * Verify that the "store" value we've been given makes sense. A store
 * declaration has two parts, whether it is global or not, and whether it is an
 * associative array or not. Currently only thread and global are allowed types
 * for storage.
 *
...
function mdGenerateSingleGather(st, name, num)
{
	...
	if (info['type'] == 'thread')
		pname = sprintf('self->%s%d', name, num);
	else
		pname = sprintf('%s%d', name, num);
...
function mdValidateMetaD(desc)
{
	...
	for (ii = 0; ii < probedesc.length; ii++) {
		...
		if ('local' in ks) {
			...
			for (jj = 0; jj < elt['local'].length; jj++) {
				...
				ASSERT(typeof (ent) == typeof ({}),
				    sprintf('local should be an array of ' +
				    'objects: %j', ent));
				key = Object.keys(ent);
				ASSERT(key.length == 1,
		...
		if ('aggregate' in ks) {
			...
			for (key in elt['aggregate']) {
				...
				ASSERT(key in allfields,
				    ...
				    key);
				ASSERT(desc['fields_internal'].indexOf(key) ==
				    -1, 'cannot aggregate an internal field');
		...
		if ('clean' in ks) {
			...
			ASSERT(!mod_jsprim.isEmpty(elt['clean']),
 ...
 *
 *	desc		See argument to mdGenerateDScript.
 *
 *	metric		See argument to mdGenerateDScript.
 *
 *	metadata	See argument to mdGenerateDScript.
 */
function mdSanityCheck(desc, metric, metadata)
{
	...
	ASSERT('predicate' in metric, 'metric object missing \'predicate\'');
	ASSERT('decomposition' in metric, 'metric object missing ' +
	    '\'decomposition\'');

	ASSERT(typeof (metric['predicate']) == typeof ({}), 'metric.' +
	    'predicate must be an object');
	ASSERT(Array.isArray(metric['decomposition']), 'metric.' +
	    'decomposition must be an array');
	ASSERT(metric['decomposition'].length <= 2,
	    'too many decompositions');

	metric['decomposition'].forEach(function (field, ii) {
		ASSERT(metadata.hasOwnProperty(field),
		    sprintf('metric.decomposition[%d] ("%s") has no metadata',
		        ii, field));
	});
...
{
	return (mod_dutil.dUtilGeneratePragma('zone', zone));
}
...
{
	var key;
	return mod_dutil.dUtilAndPredArray(local.map(function (obj) {
		for (key in obj)
			break;
		return (sprintf('((this->%s = %s) != NULL || 1)', key,
		    obj[key]));
	}));
}
...
function mdGeneratePredicate(pred)
{
	...
	return (mod_dutil.dUtilGeneratePredicate(pred));
}
...
function mdTransformsResolveEntry(trans, names)
{
	...
	trans = trans.replace(/\$hostname/g, mod_os.hostname());
...
function mdTransformsResolve(trans, gather)
{
	...
	trans = mod_jsprim.deepCopy(trans);
	for (key in gather) {
		if (!(key in trans))
			continue;
	...
	for (key in trans) {
		...
		trans[key] = sprintf('(%s)',
		    trans[key].replace(/\$hostname/g, hostname));
	}
...
function mdGenerateAggPreds(gather)
{
	...
	return (mod_dutil.dUtilAndPredArray(gather.map(function (n) {
	    return (sprintf('(%s != NULL)', n));
	})));
}
 ...
 *
 * 	decomps		A mapping of field keys to the types "number" or
 * 			"string"
 *
...
function mdGenerateAggregate(decomps, agg, trans)
{
	...
	return ({
	    script: mod_dutil.dUtilGenerateAggregate(index, agginst),
	    zero: zero,
	    hasdists: hasdists,
...
function mdGenerateClean(st)
{
	...
	return (st.map(function (x) {
	    return (mod_dutil.dUtilZeroVariable(x));
	}).join(''));
}
 ...
 *
 *	desc		The Meta-D description of a family of related metrics.
 *			This includes "fields" (an array of fields that may be
 *			used in a predicate or decomposition) and "metad", which
 *			is described in the README.
 *
 *	metric		Parameters specific to this D script, including
 *			"predicate" (a node-krill predicate), "decomposition"
 *			(a list of fields to be decomposed by), and an optional
 *			"zones" field specifying the specific set of zones to
 *			instrument.  "zones" overrides any zone-related
 *			conditions in the predicate.
 *
 *	metadata	Maps each field name used by either "predicate" or
 *			"decomposition" to a node-krill type, either "number" or
 *			"string".
 *
...
function mdGenerateDScript(desc, metric, metadata)
{
	...
	mdSanityCheck(desc, metric, metadata);
	...
	if (metric.zones) {
		...
		if (desc['metad']['usepragmazone'] &&
		    metric.zones.length < md_pragma_maxzones)
			pragmazone = true;

		zonepred = mod_dutil.dUtilOrPredArray(
		    metric.zones.map(function (elt) {
			return ('zonename == "' + elt + '"');
		    }));
	}
	 ...
	 */
	for (ii = 0; ii < metric['decomposition'].length; ii++) {
		tmp = metric['decomposition'][ii];
		decomps[tmp] = metadata[tmp];
		fields[tmp] = true;
	}

	if (!metric.predicate.trivial()) {
		tmp = metric.predicate.fields();
	...
	for (ii = 0; ii < desc['metad']['probedesc'].length; ii++) {
		...
		if ('aggregate' in pent &&
		    !metric.predicate.trivial()) {
			lpred = metric.predicate.replaceFields(ltrans);
			preds.push(lpred.toCStyleString());
		}
	...
	if (pragmazone && metric.zones) {
		ret = metric.zones.map(function (zone) {
