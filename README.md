...
Meta-D is a declarative way to describe a DTrace based metric. From a Meta-D description of a metric, we can generate several D scripts depending on what *fields* the user wants to filter on and break out the results by.  We'll start off with an example and then go and describe all the fields and what is required:
